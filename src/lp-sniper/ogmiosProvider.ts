...
export class OgmiosProvider implements Provider {
  ...
  async getUtxos(addressOrCredential: any): Promise<UTxO[]> {
    let blockfrost = new Blockfrost(
      'https://cardano-mainnet.blockfrost.io/api/v0',
      process.env.BLOCKFROST_KEY
    )
  ...
  getUtxosWithUnit(addressOrCredential: any, unit: Unit): Promise<UTxO[]> {
    let blockfrost = new Blockfrost(
      'https://cardano-mainnet.blockfrost.io/api/v0',
      process.env.BLOCKFROST_KEY
    )
  ...
  async blockfrostUtxosToUtxos(result: any) {
    return await Promise.all(
      result.map(async (r: any) => ({
        txHash: r.tx_hash,
        outputIndex: r.output_index,
        assets: Object.fromEntries(
          r?.amount?.map(({ unit, quantity }: any) => [
            unit,
            BigInt(quantity),
          ]) || []
        ),
